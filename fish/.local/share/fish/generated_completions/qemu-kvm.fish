...
complete -c qemu-kvm -o no-hpet -d 'Disable HPET support.  Deprecated, use \\(aq-machine hpet=off\\(aq instead'
...
complete -c qemu-kvm -o mon -d 'Set up a monitor connected to the chardev name'
...
complete -c qemu-kvm -o singlestep -d 'This is a deprecated synonym for the TCG accelerator property one-insn-per-tb'
...
complete -c qemu-kvm -o chroot -d 'Deprecated, use \\(aq-run-with chroot=. \\(aq instead'
...
complete -c qemu-kvm -o semihosting-config -d 'Enable and configure \\%Semihosting (ARM, M68K, Xtensa, MIPS, Nios II, RISC-V …'
...
complete -c qemu-kvm -o async-teardown -d 'This option is deprecated and should no longer be used'
complete -c qemu-kvm -o run-with -d 'Set QEMU process lifecycle options'
...
complete -c qemu-kvm -o perfmap -d 'Generate a map file for Linux perf tools that will allow basic profiling info…'
complete -c qemu-kvm -o jitdump -d 'Generate a dump file for Linux perf tools that maps basic blocks to symbol na…'
